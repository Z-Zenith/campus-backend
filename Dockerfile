...
# nosemgrep: dockerfile.security.missing-user-entrypoint -- false positive: the rule can't see
# that docker-entrypoint.sh itself unconditionally setpriv-drops to $APP_UID before exec'ing
# the app (see that script's comment) -- root here is transient, container-start-only, never
# the app's actual runtime user.
